    ...
    pub const fn entry(self) -> RuleEntry {
        ...
        match self {
            ...
            A0417 => RuleEntry {
                ...
                severity: Error,
                spec_anchor: "§7.1",
                gloss: "A component's crate has its own ply.yaml, so this box would link to it, but that file could not be read or does not parse as a valid ply.yaml document. The box keeps its own declared interior, and the command returns this finding with any independent results.",
            },
            W0532 => RuleEntry {
                code: self,
                tier: Crate,
            ...
            E0211 => RuleEntry {
                ...
                severity: Error,
                spec_anchor: "§6",
                gloss: "A linked verification cannot map one child result into the root without guessing -- for example the child document has claims under another top-level owner or an id collides while rebasing. The affected component admits no linked evidence; the command still returns the ordinary diagnostics envelope and may publish a view of that refusal alongside independent results.",
            },
            W0533 => RuleEntry {
                code: self,
                tier: Crate,
                ...
                severity: Error,
                spec_anchor: "§7.1",
                gloss: "Two components in the same document would both link to the same other document. Choosing either by declaration order would guess which evidence belongs where, so neither link forms and the command returns an error diagnostic.",
            },
            W0534 => RuleEntry {
                code: self,
                tier: Crate,
                ...
                severity: Error,
                spec_anchor: "§7.1",
                gloss: "Following a chain of cross-document links from this component would eventually revisit a document already on that chain. The link does not form and the command returns an error diagnostic, so drawing always terminates without guessing where the chain should stop.",
            },
            A0410 => RuleEntry {
                code: self,
                tier: Crate,
            ...
            W0540 => RuleEntry {
                ...
                severity: Warning,
                spec_anchor: "§5.3",
                gloss: "Ply could not fully compare a module boundary because part of the relevant source, ownership, or reference path was unresolved. The run names what it could not follow and does not present the affected rule as clean.",
            },
            ...
            W0530 => RuleEntry {
                ...
                severity: Warning,
                spec_anchor: "§5.4c",
                gloss: "Ply could not work out the whole list of functions this claim's checks run, so mutation testing planted its deliberate bugs in the part it could identify and nothing else -- a clean result covers less code here than it does when the list is complete, and this names the part Ply could not read.",
            },
            W0503 => RuleEntry {
                code: self,
                tier: Contract,
