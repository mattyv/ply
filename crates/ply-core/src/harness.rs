...
/// Names used by declared contracts retain the checked function's module
/// context, in both scalar wrappers and explicit byte-slice proofs.
fn bounded_scope_imports(cf: &ContractFn) -> String {
    let import_path = cf.import_path();
    let Some((module, _)) = import_path.rsplit_once("::") else {
        return String::new();
    };
    let mut imports = format!("#[cfg(kani)]\nuse super::{module}::*;\n");
    // An explicit opaque return-name import takes precedence over the root glob,
    // so a root enum sharing the local enum's name cannot capture the wrapper.
    if let RustType::Unsupported(name) = &cf.return_type
        && let Ok(path) = syn::parse_str::<syn::Path>(name)
        && path.leading_colon.is_none()
        && path.segments.len() == 1
        && path.segments[0].ident != "Self"
    {
        let binding = path.segments[0].ident.to_string();
        if let Some(segments) = cf.use_aliases.get(&binding) {
            let context = module.split("::").map(str::to_string).collect::<Vec<_>>();
            let full = match segments.first().map(String::as_str) {
                Some("crate" | "self" | "super") => resolve_absolute_path(&context, segments)
                    .map(|parts| format!("crate::{}", parts.join("::"))),
                _ => Some(segments.join("::")),
            };
            if let Some(full) = full {
                imports.push_str(&format!("#[cfg(kani)]\nuse {full} as {binding};\n"));
            }
        } else {
            imports.push_str(&format!("#[cfg(kani)]\nuse super::{module}::{binding};\n"));
        }
    }
    imports
}
...
pub fn generate_proof_module(
    ...
    let scoped_import = bounded_scope_imports(cf);
...
const PROOF_MODULE_HEADER: &str = "//! Generated by Ply -- do not edit. Kani proof harness for `";
const PROOF_MODULE_SUFFIX: &str =
    "\n// Ply-generated module declaration -- do not edit this line.\nmod ply_generated;\n";
const COUNTEREXAMPLE_MODULE_SUFFIX: &str =
    "\n// Ply-generated module declaration -- do not edit this line.\nmod ply_generated_cex;\n";
const DISOWNED_PROOF_MODULE_HEADER: &str = "//! Ply generated this proof module, but it changed during verification; Ply will preserve it.\n";

fn without_owned_proof_declaration(source: &str) -> Option<String> {
    let parsed = syn::parse_file(source).ok()?;
    let is_plain_module = |item: &syn::Item, expected: &str| {
        let syn::Item::Mod(module) = item else {
            return false;
        };
        module.attrs.is_empty() && module.ident == expected && module.content.is_none()
    };

    if is_plain_module(parsed.items.last()?, "ply_generated") {
        return source
            .strip_suffix(PROOF_MODULE_SUFFIX)
            .map(ToString::to_string);
    }

    // Older Ply versions could publish a durable counterexample after
    // leaving the proof scratch in place. Remove only the preceding proof
    // pair and retain the generated test and its declaration.
    if parsed.items.len() >= 2
        && is_plain_module(&parsed.items[parsed.items.len() - 2], "ply_generated")
        && is_plain_module(parsed.items.last()?, "ply_generated_cex")
    {
        let before_counterexample = source.strip_suffix(COUNTEREXAMPLE_MODULE_SUFFIX)?;
        let original = before_counterexample.strip_suffix(PROOF_MODULE_SUFFIX)?;
        return Some(format!("{original}{COUNTEREXAMPLE_MODULE_SUFFIX}"));
    }

    None
}

/// Removes a proof module left by an older or interrupted Ply run.
///
/// Only a file carrying Ply's generated-proof header is removed. A user-owned
/// `ply_generated.rs` is left untouched, including its module declaration.
pub fn prune_stale_proof_module(crate_src_dir: &Path, lib_path: &Path) -> Result<bool> {
    let module_path = crate_src_dir.join("ply_generated.rs");
    let module_metadata = match std::fs::symlink_metadata(&module_path) {
        Ok(metadata) => Some(metadata),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => None,
        Err(error) => {
            return Err(error).with_context(|| format!("inspecting {}", module_path.display()));
        }
    };
    // Never follow a user-owned symlink (including a dangling one), and do
    // not claim ownership of directories or other special filesystem nodes.
    if module_metadata
        .as_ref()
    ...
    let module_source = match std::fs::read_to_string(&module_path) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => None,
        Err(error) => {
            return Err(error).with_context(|| format!("reading {}", module_path.display()));
        }
    };
    ...
    let lib_source = match std::fs::read_to_string(lib_path) {
        Ok(source) => Some(source),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => None,
        Err(error) => return Err(error).with_context(|| format!("reading {}", lib_path.display())),
    };
    let cleaned_lib = lib_source
        .as_deref()
        .and_then(without_owned_proof_declaration);
    let generated_file_exists = module_source
        .as_deref()
        .is_some_and(|source| source.starts_with(PROOF_MODULE_HEADER));

    // Delete only a pair Ply can identify at both ends. If either the file
    // or declaration was reformatted, attributed, moved, or otherwise
    // edited, ownership is ambiguous and verification refuses to overwrite
    // it. A missing file plus the exact final declaration is the one safe
    // half-written state to clean on its own.
    if cleaned_lib.is_none() || (module_source.is_some() && !generated_file_exists) {
        return Ok(false);
    }

    std::fs::write(lib_path, cleaned_lib.unwrap()).with_context(|| {
        format!(
            "removing the stale Ply declaration from {}",
            lib_path.display()
        )
    })?;
    if generated_file_exists {
        std::fs::remove_file(&module_path)
            .with_context(|| format!("removing stale {}", module_path.display()))?;
    }
    Ok(true)
}

/// A serial proof module installed in the user's crate for only as long as
/// Kani needs it. Drop restores the exact bytes Ply observed after pruning
/// any recognized stale output from an older run.
pub struct TemporaryProofModule {
    module_path: PathBuf,
    lib_path: PathBuf,
    original_lib: Vec<u8>,
    written_lib: Vec<u8>,
    written_module: Vec<u8>,
    active: bool,
}

impl TemporaryProofModule {
    fn restore_inner(&mut self) -> Result<()> {
        let module_now = match std::fs::read(&self.module_path) {
            Ok(bytes) => Some(bytes),
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => None,
            Err(error) => {
                return Err(error)
                    .with_context(|| format!("reading {}", self.module_path.display()));
            }
        };
        if module_now
            .as_deref()
            .is_some_and(|bytes| bytes != self.written_module)
        {
            // Persist the decision to preserve a concurrently edited proof
            // module. Replacing only Ply's ownership comment prevents a
            // later run from mistaking the edited file for stale scratch.
            let module_now = module_now.as_ref().unwrap();
            if module_now.starts_with(PROOF_MODULE_HEADER.as_bytes())
                && let Some(line_end) = module_now.iter().position(|byte| *byte == b'\n')
            {
                let mut disowned = DISOWNED_PROOF_MODULE_HEADER.as_bytes().to_vec();
                disowned.extend_from_slice(&module_now[line_end + 1..]);
                std::fs::write(&self.module_path, disowned).with_context(|| {
                    format!("preserving the edited {}", self.module_path.display())
                })?;
            }
            self.active = false;
            bail!(
                "{} changed while the proof was running; Ply preserved it and did not publish a partial verification result",
                self.module_path.display()
            );
        }
        let lib_now = std::fs::read(&self.lib_path)
            .with_context(|| format!("reading {}", self.lib_path.display()))?;
        let lib_changed = lib_now != self.written_lib;
        let restore = if !lib_changed {
            Some(self.original_lib.clone())
        } else {
            std::str::from_utf8(&lib_now)
                .ok()
                .and_then(without_owned_proof_declaration)
        ...
        let Some(restore) = restore else {
            self.active = false;
            bail!(
                "{} changed while the proof was running and Ply could not identify its temporary module declaration; both files were preserved and no partial verification result was published",
                self.lib_path.display()
            );
        };
        // Restore the declaration first. If removal then fails, an
        // unreferenced generated file is harmless; the inverse order could
        // leave an ordinary cargo build pointing at a missing module.
        std::fs::write(&self.lib_path, restore)
            .with_context(|| format!("restoring {} after the proof", self.lib_path.display()))?;
        if module_now.is_some() {
            std::fs::remove_file(&self.module_path)
                .with_context(|| format!("removing temporary {}", self.module_path.display()))?;
        }
        self.active = false;
        if lib_changed {
            bail!(
                "{} changed while the proof was running; Ply preserved the edit, removed its temporary proof module, and did not publish a partial verification result",
                self.lib_path.display()
            );
        }
        Ok(())
    ...
    /// Restores the user's source tree and reports an ambiguous edit or I/O
    /// failure instead of publishing a result over a dirty workspace.
    pub fn restore(mut self) -> Result<()> {
        self.restore_inner()
    }
}

impl Drop for TemporaryProofModule {
    fn drop(&mut self) {
        if self.active {
...
/// Installs one temporary serial proof module and returns its cleanup guard.
pub fn install_temporary_proof_module(
    crate_src_dir: &Path,
    lib_path: &Path,
    module_source: &str,
) -> Result<TemporaryProofModule> {
    prune_stale_proof_module(crate_src_dir, lib_path)?;
    let module_path = crate_src_dir.join("ply_generated.rs");
    match std::fs::symlink_metadata(&module_path) {
        Ok(_) => {
            bail!(
                "Ply will not overwrite user-owned source at {}",
                module_path.display()
            );
        }
    ...
    let install_result = (|| -> Result<()> {
        let mut file = std::fs::OpenOptions::new()
            ...
            .open(&module_path)
            .with_context(|| {
                format!("creating {} without overwriting it", module_path.display())
            })?;
        created_module = true;
        std::io::Write::write_all(&mut file, module_source.as_bytes())
            .with_context(|| format!("writing {}", module_path.display()))?;
        ensure_generated_declaration(lib_path, "ply_generated")
    })();
    if let Err(error) = install_result {
        let _ = std::fs::write(lib_path, &original_lib);
        if created_module {
    ...
    let written_lib =
        std::fs::read(lib_path).with_context(|| format!("reading {}", lib_path.display()))?;
    let written_module = std::fs::read(&module_path)
        .with_context(|| format!("reading {}", module_path.display()))?;
    Ok(TemporaryProofModule {
        module_path,
...
fn write_generated_file(
    ...
    ensure_generated_declaration(lib_path, file_stem)?;
    Ok(out_path)
}

fn ensure_generated_declaration(lib_path: &Path, file_stem: &str) -> Result<()> {
...
"#,
    ...
    #[test]
    fn proof_codegen_keeps_assumed_and_verified_callee_stubs_distinct() {
        let dir = tempfile::tempdir().unwrap();
        let path = write_src(
            dir.path(),
            r#"
#[ply::ensures(|result| *result == x)]
pub fn caller(x: u32) -> u32 { x }
"#,
        );
        let cf = discover_fn(&path, "caller").unwrap();
        let raw_params = vec![("x".to_string(), "u32".to_string())];
        let normalized_params = vec![Param {
            name: "x".into(),
            ty: RustType::U32,
            by_ref: false,
        }];

        let assumed = StubSpec {
            callee_path: "legacy_rate".into(),
            params: raw_params.clone(),
            return_type: "u32".into(),
            requires: vec![],
            ensures: vec!["|result| *result <= x".into()],
            kind: StubKind::Assumed,
        };
        let assumed_source = generate_proof_module(&cf, 2, &[assumed])
            .unwrap()
            .module_source;
        assert!(assumed_source.contains("#[kani::stub(legacy_rate, ply_stub_legacy_rate)]"));
        assert!(!assumed_source.contains("#[kani::stub_verified(legacy_rate)]"));

        let verified = StubSpec {
            callee_path: "g".into(),
            params: raw_params,
            return_type: "u32".into(),
            requires: vec![],
        ...
        let verified_source = generate_proof_module(&cf, 2, &[verified])
            .unwrap()
            .module_source;
        assert!(verified_source.contains("#[kani::stub_verified(g)]"));
        assert!(verified_source.contains("#[kani::proof_for_contract(g)]"));
        assert!(!verified_source.contains("#[kani::stub(g,"));
    }
    ...
    #[test]
    fn temporary_proof_module_restores_the_crate_when_the_guard_drops() {
        let dir = tempfile::tempdir().unwrap();
        let src_dir = dir.path();
        let lib_path = write_src(src_dir, "pub fn f() {}\n");
        let original = std::fs::read(&lib_path).unwrap();

        {
            let _guard = install_temporary_proof_module(
                src_dir,
                &lib_path,
                "//! Generated by Ply -- do not edit. Kani proof harness for `f`\n",
            )
            .unwrap();
            assert!(src_dir.join("ply_generated.rs").exists());
            assert!(
                std::fs::read_to_string(&lib_path)
                    .unwrap()
                    .contains("mod ply_generated;")
            );
        }

        assert_eq!(std::fs::read(&lib_path).unwrap(), original);
        assert!(!src_dir.join("ply_generated.rs").exists());
    }

    #[test]
    fn stale_proof_module_and_its_declaration_are_pruned() {
        let dir = tempfile::tempdir().unwrap();
        let src_dir = dir.path();
        let lib_path = write_src(
            src_dir,
            "pub fn f() {}\n\n// Ply-generated module declaration -- do not edit this line.\nmod ply_generated;\n",
        );
        std::fs::write(
            src_dir.join("ply_generated.rs"),
            "//! Generated by Ply -- do not edit. Kani proof harness for `old`\n",
        )
        .unwrap();

        assert!(prune_stale_proof_module(src_dir, &lib_path).unwrap());
        assert_eq!(
    ...
    #[test]
    fn stale_proof_before_counterexample_module_keeps_the_counterexample() {
        let dir = tempfile::tempdir().unwrap();
        let src_dir = dir.path();
        let counterexample_declaration = "\n// Ply-generated module declaration -- do not edit this line.\nmod ply_generated_cex;\n";
        let lib_path = write_src(
            src_dir,
            &format!(
                "pub fn f() {{}}\n\n// Ply-generated module declaration -- do not edit this line.\nmod ply_generated;\n{counterexample_declaration}"
            ),
        );
        std::fs::write(
            src_dir.join("ply_generated.rs"),
        ...
        .unwrap();
        std::fs::write(
            src_dir.join("ply_generated_cex.rs"),
            "#[test]\nfn durable_counterexample() {}\n",
        )
        .unwrap();
        ...
        assert!(!src_dir.join("ply_generated.rs").exists());
        assert_eq!(
            std::fs::read_to_string(src_dir.join("ply_generated_cex.rs")).unwrap(),
            "#[test]\nfn durable_counterexample() {}\n"
        );
    }

    #[test]
    fn a_user_owned_module_with_the_same_name_is_never_deleted() {
        let dir = tempfile::tempdir().unwrap();
        let src_dir = dir.path();
        let lib_path = write_src(src_dir, "pub fn f() {}\nmod ply_generated;\n");
        std::fs::write(
            src_dir.join("ply_generated.rs"),
            "pub fn belongs_to_the_user() {}\n",
        )
        .unwrap();
    ...
    #[cfg(unix)]
    #[test]
    fn a_dangling_symlink_at_the_proof_path_is_never_followed_or_deleted() {
        use std::os::unix::fs::symlink;

        let dir = tempfile::tempdir().unwrap();
        let src_dir = dir.path();
        let lib_path = write_src(src_dir, "pub fn f() {}\n");
        let module_path = src_dir.join("ply_generated.rs");
        let target_path = src_dir.join("missing-user-target.rs");
        symlink(&target_path, &module_path).unwrap();

        let error = match install_temporary_proof_module(
            src_dir,
            &lib_path,
            "//! Generated by Ply -- do not edit. Kani proof harness for `f`\n",
        ) {
            Ok(_) => panic!("a user-owned symlink must block installation"),
            Err(error) => error,
        };
        ...
        assert!(
            std::fs::symlink_metadata(&module_path)
                .unwrap()
                .file_type()
                .is_symlink()
        );
    ...
    #[cfg(unix)]
    #[test]
    fn stale_pruning_never_infers_ownership_through_a_symlink() {
        use std::os::unix::fs::symlink;

        let dir = tempfile::tempdir().unwrap();
        let src_dir = dir.path();
        let lib_source = "pub fn f() {}\n\n// Ply-generated module declaration -- do not edit this line.\nmod ply_generated;\n";
        let lib_path = write_src(src_dir, lib_source);
        let target_path = src_dir.join("user-target.rs");
        std::fs::write(
            &target_path,
            "//! Generated by Ply -- do not edit. Kani proof harness for `old`\n",
        )
        ...
        assert!(target_path.exists());
        assert_eq!(std::fs::read_to_string(&lib_path).unwrap(), lib_source);
    }

    #[test]
    fn temporary_proof_cleanup_never_overwrites_an_edit_made_during_the_run() {
        let dir = tempfile::tempdir().unwrap();
        let src_dir = dir.path();
        let lib_path = write_src(src_dir, "pub fn f() {}\n");

        let guard = install_temporary_proof_module(
            src_dir,
            &lib_path,
            "//! Generated by Ply -- do not edit. Kani proof harness for `f`\n",
        )
        ...
        assert!(
            std::fs::read_to_string(&lib_path)
                .unwrap()
                .contains("added_while_kani_ran")
        );
        assert!(
            src_dir.join("ply_generated.rs").exists(),
            "leaving the module is safer than breaking a concurrent edit that still declares it"
        );
        assert!(!prune_stale_proof_module(src_dir, &lib_path).unwrap());
        assert!(
            src_dir.join("ply_generated.rs").exists(),
            "a later run must preserve the same ambiguous pair rather than leave a dangling mod"
        );
    }

    #[test]
    fn explicit_restore_raises_an_error_when_the_crate_edit_is_ambiguous() {
        let dir = tempfile::tempdir().unwrap();
        let src_dir = dir.path();
        let lib_path = write_src(src_dir, "pub fn f() {}\n");
        ...
        std::fs::write(
            &lib_path,
            "pub fn f() {}\nmod ply_generated;\npub fn concurrent_edit() {}\n",
        )
        .unwrap();

        let error = guard.restore().expect_err("ambiguous ownership must abort");
        assert!(error.to_string().contains("both files were preserved"));
        assert!(src_dir.join("ply_generated.rs").exists());
        assert!(
            std::fs::read_to_string(&lib_path)
                .unwrap()
    ...
    #[test]
    fn explicit_restore_rejects_a_safe_concurrent_edit_after_cleaning_scratch() {
        let dir = tempfile::tempdir().unwrap();
        let src_dir = dir.path();
        let lib_path = write_src(src_dir, "pub fn f() {}\n");
        ...
        .unwrap();
        let installed = std::fs::read_to_string(&lib_path).unwrap();
        std::fs::write(&lib_path, installed.replace("fn f", "fn renamed")).unwrap();

        let error = guard
            .restore()
            .expect_err("a changed verification input must reject the proof result");
        assert!(
            error
                .to_string()
                .contains("did not publish a partial verification result"),
            "{error:#}"
        );
        assert_eq!(
            std::fs::read_to_string(&lib_path).unwrap(),
    ...
    #[test]
    fn a_proof_module_edited_during_the_run_is_disowned_and_preserved() {
        let dir = tempfile::tempdir().unwrap();
        let src_dir = dir.path();
        let lib_path = write_src(src_dir, "pub fn f() {}\n");
        ...
        let preserved = std::fs::read_to_string(src_dir.join("ply_generated.rs")).unwrap();
        assert!(preserved.contains("fn user_edit()"));
        assert!(preserved.starts_with(DISOWNED_PROOF_MODULE_HEADER));
        assert!(!prune_stale_proof_module(src_dir, &lib_path).unwrap());
    }

    #[test]
    fn a_marker_like_string_or_attributed_module_is_not_pruned() {
        for lib_source in [
            "const TEXT: &str = r#\"\n// Ply-generated module declaration -- do not edit this line.\nmod ply_generated;\n\"#;\n",
            "#[cfg(kani)]\n\n// Ply-generated module declaration -- do not edit this line.\nmod ply_generated;\n",
        ] {
