...
pub struct Node {
    ...
    /// and `ensures` clauses behind the verdict.
    ///
    ...
    /// carries clauses -- and an envelope-driven canvas could not have
    /// drawn the contract mark from this without them. Additive, so a
    /// reader of the old shape is unaffected (§8's stability rule).
    #[serde(default, skip_serializing_if = "Contract::is_empty")]
    pub contract: Contract,
    /// Claims a human vouched for and no engine checked (§5.4d), with the
    /// evidence named for each. Carried here for the same reason: `audit`
    /// has always reported the trust surface, but the tree a machine reads
    /// never said which node was standing on someone's word.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
