...
them discarded by the parser, which is the format inviting prose and then throwing it
away.

It is deliberately not offered on a function, and the reason is a failure observed in
use: someone wrote a real invariant as an `examples` string -- a test case wearing a
specification's clothes -- because no better slot was visible. The answer there is
`ensures`, which both a reader and an engine can act on. A prose slot beside it would
make that mistake comfortable rather than correcting it: the next invariant lands in the
note and never becomes a promise. What a function does belongs in its contract, and where
there is no contract the drawing already says so -- a bare chip, an unfilled box.
...
so a carried-forward verdict says the same thing a freshly earned one does: a promise is
a property of the claim and does not stop existing because a result was reused. Additive,
so a reader of the old shape is unaffected.
