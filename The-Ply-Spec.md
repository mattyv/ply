...
| D6 | Verdicts aggregate upward as **worst-of** over the evidence order `violation < unclaimed < tested < fuzzed < bounded < proved`. Statuses (`conditional`, `owed-evidence`, `weak-spec`, `unsupported`, `engine-missing`, `timeout`, `inconclusive`) do not sit in that order; they propagate upward as flags and open-item counts alongside it. **`tool_error`, `timeout` and `unsupported` are also reported as verdicts by `cargo ply verify`, ranked between `violation` and `unclaimed`** — an extension to this order, recorded here on 2026-09-06 rather than left as an undocumented divergence. "The engine gave up" is more use to a reader at a glance than a bare `unclaimed`, and nothing is lost by it, because each of the three *also* travels upward as a flag: before that was true, a `timeout` sitting beside a `violation` lost the worst-of and vanished from the report entirely, so fixing the broken promise turned the component green over a function nobody had checked. The extension is why `ply_core::kernel::aggregate` cannot simply be called by `verify` — the kernel has no representation for these three — and the six rungs that *are* shared are bound to it instead: `verify`'s ladder carries `kernel::Evidence` itself rather than restating the order, and a bounded differential folds every small tree both ways and requires the same answer. `owed-evidence` is the debt half of `conditional` (§5.5, added to this list 2026-08-25 — it was being emitted before it was defined): `conditional` says the verdict rests on an assumed contract, `owed-evidence` says nothing has yet checked that contract against the real body. They are two facts, and a run that discharges the second keeps the first. | A proof in one corner must not hide a merely-tested boundary in another; and a timeout is not a weaker proof, it is a different kind of fact. |
...
- **`&[&[u8]]`** — native `bounded(k)` support for up to k rows, each up to k
  arbitrary bytes, built from fixed stack arrays with independent symbolic lengths.
  `K0510` reports the disjoint backing domain; shared-storage aliasing and pointer
  identity are outside it. This shape uses a generated `#[kani::proof]` that assumes
  the merged precondition, calls the real body, and explicitly asserts the merged
  postcondition. Inline postconditions are expanded only in a private source shadow;
  the application body stays unchanged and dependency contract assertions stay enabled.
  Recognized bounded `all` expressions use equivalent finite expansion
  to avoid the iterator-model path; other expressions retain their Rust code. Ordinary
  counterexample tests evaluate the original postcondition. General slices, including
  `&[String]`, still have no native bounded generator;
...
| assumption chain | thin dotted arrows from a verdict to the contracts it assumed — **designed, not drawn.** `tools/render` emits exactly two arrow kinds (`edge-call`, `edge-flow`) plus the derived `entry` arrow and the deny bar; no dotted rail exists in any render. Assumption chains reach a reader through `cargo ply tree` and the text form only. Retracted from README.md's line table on 2026-09-07, where it had been listed as a mark a reader would see (TODO.md) |
...
| module-anchored components | one compact outlined band inside the workspace, titled `Architecture · module boundaries inside crates`. It appears only when this drawing contains a module anchor. Its row counts those boundaries and says `declared` before a run, green only when a source-reference check actually covered them, red when that check found a forbidden reference, and dashed blue when a completed run did not check them or could not finish. The component boxes and permitted/forbidden arrows retain their existing shapes; this band answers the separate question “did Ply inspect crossings between these modules?” |
| `acceptance:` | a separate outlined band inside the workspace, titled `Application acceptance · finite production-path examples`, with one labelled row per claim. The author-written requirement appears beneath the result, wrapped by display width to at most two lines so one paragraph or wide-script string cannot dominate the drawing; each line also carries an explicit SVG pixel-length bound so a browser's fallback font cannot paint it outside the band. The complete requirement remains in the row tooltip. Its dot reads declared in a declaration view, green only after the exact test passed, red on a product failure, and dashed blue for timeout/tool/not-run gaps. It never changes a component ceiling or fn-chip fill. The tooltip also names the component, production entry, exact Cargo test, fixture and oracle paths, and states that the evidence is finite. |
| drawing provenance | the opening verdict strip begins `Declaration view` for a document-only `cargo ply render` drawing and `Evidence view` for a completed `cargo ply verify --svg` drawing. The frame tooltip repeats the command and distinguishes result colours from red bars, which always depict declared prohibitions. Colour is not the only way to tell a promise from evidence. |
