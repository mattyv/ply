...
Both were handed to this session as "found pointing Ply at `semver`, see
`docs/reach-measurement-2.md`" -- that file does not exist in this checkout
(no such measurement report is committed anywhere in the repo). The two
reproductions were taken at face value and fixed; the citation is recorded
here rather than silently dropped, and rather than this session fabricating
a measurement report it did not itself run.
