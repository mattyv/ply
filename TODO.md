...
- [ ] Extend coverage, JSON, explanations, and drawings so package evidence, source
  ...
  - [ ] Draw each acceptance claim's requirement beneath its result without letting long
        prose dominate the canvas, retain the full requirement on hover, and label every
        SVG as a declaration view or an evidence view.
...
- [x] **The real fix: `verify` follows the link** (landed in `4cad5d4`), one level,
      ...
      a client whose rule is never to recalculate; folding the interior back hides the
      problem and gives up what expanding it bought. The kernel needs no new rule -- graft
