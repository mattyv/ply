...
fn a_two_thirds_rejection_rate_raises_the_high_rejection_warning() {
    ...
    assert_eq!(warnings.len(), 1, "expected exactly one high-rejection warning: {}", run.json);
    let title = warnings[0]["title"].as_str().unwrap();
    assert!(
        title.starts_with("most of the inputs generated for `mostly_rejected` were thrown away by its own \
    ...
    assert_eq!(warnings[0]["severity"], "warning", "a weak-evidence warning is not a failure: {}", run.json);
