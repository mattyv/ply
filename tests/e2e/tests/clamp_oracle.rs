...
fn rendered_cex_test_fails_for_noncrashing_ensures_violation_then_passes_after_fix() {
    ...
    assert!(
        diag["counterexample"]["cargo_test"].is_string(),
        "must carry a cargo_test artifact path"
    );
    assert!(
        diag["counterexample"]["kani_witness"].is_string(),
        "must carry kani_witness (D7 rename), never kani_playback"
    );

    let cex_path = fixture.path().join("src/ply_generated_cex.rs");
    assert!(
        cex_path.is_file(),
        "expected a generated ply_cex_clamp_* test file"
    );
    let cex_source = std::fs::read_to_string(&cex_path).unwrap();
    assert!(
        cex_source.contains("ply_cex_clamp_"),
        "test name must start with ply_cex_ (housekeeping ownership marker)"
    );
    ...
    assert!(
        !test_run.success,
        "the cex test must fail before the fix:\n{}",
    ...
    let fixed = fixture
        .read_lib_rs()
        .replace("*result == x)", "*result == x.min(100))");
    assert_ne!(
        fixed,
        fixture.read_lib_rs(),
    ...
    assert_eq!(
        run2.json["root"]["verdict"], "bounded(2)",
        "envelope: {}",
        run2.json
    );
    ...
    assert!(
        test_run2.success,
        "the cex test must pass after the fix:\n{}",
