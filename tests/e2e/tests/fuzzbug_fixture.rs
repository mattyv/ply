...
fn fuzz_check_shrinks_a_seeded_bug_and_renders_it_as_a_red_test_then_passes_after_fix() {
    ...
    assert_eq!(
        ...
        "proptest must shrink to the minimal seeded input"
    );
    assert!(
    ...
    assert!(
        cex_path.is_file(),
        "expected a generated ply_cex_seeded_bug_* test file"
    );
    assert!(std::fs::read_to_string(&cex_path)
        .unwrap()
        .contains("ply_cex_seeded_bug_"));
    ...
    assert_eq!(
        run2.json["diagnostics"].as_array().unwrap().len(),
        0,
        "envelope: {}",
        run2.json
    );
