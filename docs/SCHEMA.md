...
`cargo ply verify` keeps its ordinary scratch output under `target/ply/`. A serial
bounded proof temporarily installs `src/ply_generated.rs` and a marked
`mod ply_generated;` line so the model checker can see private crate items. Ply restores
`src/lib.rs` byte-for-byte and removes the scratch file when the proof ends, including on
failure or interruption; a later run also prunes recognized leftovers from older
versions. The `test`/`fuzz` tier writes a small crate under `target/ply/`. Ply owns
everything under `target/ply/` and every generated counterexample test whose name begins
with `ply_cex_`; those counterexample tests intentionally remain as runnable repair
targets.
...
| `edges:`/`deny:` between module-anchored components in the **same** crate | **Partly** — resolved source references are checked; unresolved source keeps the result incomplete | `A0402` (warning), `A0420` with `strict: true` (error), `W0540` |
...
| `A0402` | A resolved source reference crosses a module boundary without permission, or matches an explicit deny rule. Advisory unless the referring component opts into strict checks. |
| `A0420` | The same module-boundary crossing under a referring component with `strict: true`; the explicit opt-in makes it an error. |
